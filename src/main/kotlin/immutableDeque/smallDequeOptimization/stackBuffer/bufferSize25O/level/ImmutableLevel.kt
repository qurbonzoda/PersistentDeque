package immutableDeque.smallDequeOptimization.stackBuffer.bufferSize25O.level
...
import immutableDeque.smallDequeOptimization.stackBuffer.bufferSize25O.persistentDeque.DequeSubStack
import immutableDeque.smallDequeOptimization.stackBuffer.bufferSize25O.persistentDeque.PersistentDeque
...
internal abstract class ImmutableLevel(val lhs: ImmutableBuffer,
    ...
    fun <T> makeGreenUpperLevelPushingLhs(valueToPush: Any?,
...
//        assert(upper.rhs.color != RED)
        ...
        return this.makeImmutableDeque(upperLhs, upperRhs, thisLhs, thisRhs, lowerSubStack, pDeque.size + 1)
    }

    fun <T> makeGreenUpperLevelPushingRhs(valueToPush: Any?,
                                          pDeque: PersistentDeque<*>,
                                          lowerSubStack: DequeSubStack?): ImmutableDeque<T> {
...
//        assert(upper.lhs.color != RED)
    ...
    fun <T> makeGreenUpperLevelPoppingLhs(pDeque: PersistentDeque<*>, lowerSubStack: DequeSubStack?): ImmutableDeque<T> {
//        assert(upper.lhs.size == RED_LOW)
//        assert(upper.rhs.color != RED)

        val delta = if (this.color == GREEN && lowerSubStack != null && lowerSubStack.stack.color == RED) 1 else 0

        val upperLhs = this.fillUpperFromThis(pDeque.lhs.pop(), this.lhs, delta)
    ...
    fun <T> makeGreenUpperLevelPoppingRhs(pDeque: PersistentDeque<*>, lowerSubStack: DequeSubStack?): ImmutableDeque<T> {
//        assert(upper.rhs.size == RED_LOW)
//        assert(upper.lhs.color != RED)

        val delta = if (this.color == GREEN && lowerSubStack != null && lowerSubStack.stack.color == RED) 1 else 0

        val upperRhs = this.fillUpperFromThis(pDeque.rhs.pop(), this.rhs, delta)
