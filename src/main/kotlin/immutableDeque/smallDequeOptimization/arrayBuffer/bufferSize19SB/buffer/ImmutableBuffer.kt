package immutableDeque.smallDequeOptimization.arrayBuffer.bufferSize19SB.buffer

import immutableDeque.ImmutableDeque
import immutableDeque.smallDequeOptimization.arrayBuffer.bufferSize19SB.constants.*

internal abstract class ImmutableBuffer(val array: Array<Any?>): ImmutableDeque<Any?> {
    val top: Any?
        get() {
    ...
    fun push(value: Any?): ImmutableBuffer {
        ...
        return this.withArray(newArray)
    }

    fun pop(count: Int = 1): ImmutableBuffer {
//        assert(count > 0 &&  count <= this.size)

        val newArray = arrayOfNulls<Any?>(this.size - count)
...
//        assert(count > 0 && count <= this.size)
    ...
    open fun prependSavingOrder(buffer: ImmutableBuffer): ImmutableBuffer {
        val newArray = arrayOfNulls<Any?>(buffer.size + this.size)
    ...
    fun removeBottomAndMoveRestToOppositeSideBuffer(): ImmutableBuffer {
        ...
        return this.oppositeSideBufferWithArray(newArray)
    }
    ...
    abstract fun withArray(array: Array<Any?>): ImmutableBuffer
    abstract fun oppositeSideBufferWithArray(array: Array<Any?>): ImmutableBuffer
